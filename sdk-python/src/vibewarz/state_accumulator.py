...
element-wise merging:
  ...
  - **Curve**: ``trail_delta`` (list[list[(x, y)]], one per player)
    is appended element-wise onto cumulative ``trails``.

If a game adds another append-only field in the future, add its rule
to ``_APPEND_RULES`` below; the rule is a 2-tuple
``(delta_key, base_key)`` and the accumulator handles the rest.
